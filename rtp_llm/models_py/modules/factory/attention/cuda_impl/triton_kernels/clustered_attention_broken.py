"""Triton kernel for clustered decode attention optimization - Version 2.

This version uses a different approach that doesn't require fixed-size arrays
for num_clusters, making it compatible with any number of clusters.
"""
...
def fused_centroid_scoring_topp_kernel(
    ...
    scores_buffer_ptr,  # [num_heads, num_clusters_padded] - pre-allocated buffer
    num_heads: tl.constexpr,
    num_clusters: tl.constexpr,
    num_clusters_padded: tl.constexpr,  # Must be power of 2
    ...
    Uses pre-allocated scores buffer to avoid runtime array allocation.
    """
    ...
    # Compute scores for all clusters and store in buffer
    scores_offset = pid * num_clusters_padded
    ...
    for c_id in range(num_clusters):
        ...
        score = dot + log_size
        tl.store(scores_buffer_ptr + scores_offset + c_id, score)

    # Pad the rest with -inf
    for c_id in range(num_clusters, num_clusters_padded):
        tl.store(scores_buffer_ptr + scores_offset + c_id, -1e9)

    # Load all scores for softmax
    scores_mask = tl.arange(0, num_clusters_padded) < num_clusters
    scores = tl.load(
        scores_buffer_ptr + scores_offset + tl.arange(0, num_clusters_padded),
        mask=scores_mask,
        other=-1e9,
    )
    ...
    for k in range(max_selected):
        ...
        # Find the index of max (linear search within valid range)
        max_idx = 0
        for i in range(num_clusters):
            score_i = tl.load(scores_buffer_ptr + scores_offset + i)
            if i == 0:
                max_idx = 0
            else:
                # Check if this is the max
                is_max = (temp_scores[i] == max_prob) and (i < num_clusters)
                if is_max:
                    max_idx = i
                    break
        ...
        mask = tl.arange(0, num_clusters_padded) == max_idx
        temp_scores = tl.where(mask, -1e9, temp_scores)
...
def fused_centroid_scoring_topp(
    ...
    # Round up num_clusters to next power of 2 for Triton
    num_clusters_padded = triton.next_power_of_2(num_clusters)
    ...
    # Allocate temporary buffer for scores (padded to power of 2)
    scores_buffer = torch.full(
        (num_heads, num_clusters_padded),
        fill_value=-1e9,
        dtype=torch.float32,
        device=q.device,
    )
    ...
    fused_centroid_scoring_topp_kernel[grid](
        ...
        num_clusters_padded=num_clusters_padded,
        head_dim=head_dim,
...
# Benchmark helper function (same as before)
...
if __name__ == "__main__":
    # Quick test
    print("Testing Triton kernel V2...")
