    ...
    for (int i = 0; i < load_context.peer_addrs.size(); i++) {
        ...
        for (size_t layer_id = 0; layer_id < layer_num; layer_id++) {
            ...
            for (int gid_int : layer_gids) {
                ...
                // REBASE CONFLICT CONTEXT(6511f0467): source branch used a
                // direct `blockPositionsForCacheTransfer(...)` call for PD
                // debug. New base routes through `blockPositionsForLoad(...)`
                // so CP-sliced fixed regions and compact block tables map to
                // the right cache-key indices. Keep the new planner and add
                // the source debug log around its selected positions.
                if (pdDebugEnabled()) {
                ...
                for (size_t mtp_model_id = 0; mtp_model_id < propose_maga_init_params_->mtp_model_params_->size();
                    ...
                    for (size_t layer_id = 0; layer_id < layer_num; layer_id++) {
                        ...
                        for (int gid_int : mtp_layer_gids) {
                            ...
                            // `blockPositionsForLoad` to preserve CP-sliced
                            // fixed-region/cache-key mapping. Keep the planner
                            // and retain source PD debug details.
                            if (pdDebugEnabled()) {
                            ...
                            for (size_t block_pos : block_pos_list) {
                                ...
                                // REBASE CONFLICT CONTEXT(6511f0467): source branch
                                // indexed `cache_keys[block_pos]` and used
                                // peer-wide destination slicing. New base maps
                                // compact fixed-region block positions back to
                                // canonical cache-key indices and handles
                                // page-level RR local parts. Keep that mapping.
                                size_t cache_key_index = 0;
